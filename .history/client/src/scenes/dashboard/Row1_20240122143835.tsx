...
const Row1 = () => {
    ...
    const { data, status } = useGetKpisQuery<GetKpisResponse>();

    console.log('lalalal:', data);

    const revenueExpenses = useMemo(() => {
      return(
        status === 'success'  && data.monthlyData.map(({month, revenue, expenses}) => {
    ...
    const revenueProfit = useMemo(() => {
      return (
        data  && data.monthlyData.map(({month, revenue, expenses}) => {
          return {
            name: month.substring(0, 3),
            revenue: revenue,
